...
// 		a corrente circular, um filosofo nao depende do resto pra comer por ser canhoto
...
func main() {
	...
	fmt.Println("Filosofo " + strconv.Itoa(PHILOSOPHERS-2) + " canhoto!")
	go philosopher(PHILOSOPHERS-2, fork_channels[0], fork_channels[FORKS-1])
	fmt.Println("Filosofo " + strconv.Itoa(PHILOSOPHERS-3) + " canhoto!")
	go philosopher(PHILOSOPHERS-3, fork_channels[0], fork_channels[FORKS-1])
